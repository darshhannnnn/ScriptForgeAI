...
export async function safeGenerateText(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      log.debug(`generateText attempt ${attempt}/${maxRetries}`, { model: currentModelId, timeout });
      ...
      log.info(`generateText success`, { model: currentModelId, attempt, duration: elapsed });
      ...
      const elapsed = Date.now() - startTime;
      log.warn(`generateText attempt ${attempt} failed for model ${currentModelId}`, { 
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('429') || error.message.includes('rate limit') || error.message.includes('quota') || error.message.includes('limit')) {
          log.warn('Rate limited or quota issue, waiting 5 seconds...');
          await delay(5000);
        }
      ...
      // Detect transient error or model availability issues (503 / 500 / 429 / Service Unavailable / High demand / Resource exhausted / Quota / Not Found / 404)
      const errText = (String(error).toLowerCase() + ' ' + (lastError?.message || '').toLowerCase());
      const isFallbackTrigger = errText.includes('503') || 
      ...
      if (isFallbackTrigger) {
        ...
        if (nextModelId) {
          console.log(`[ai-provider] Model ${currentModelId} failed. Falling back to ${nextModelId}...`);
          currentModelId = nextModelId;
          currentModel = provider(currentModelId);
        }
      }
...
export async function safeGenerateObject<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      log.debug(`generateObject attempt ${attempt}/${maxRetries}`, { model: currentModelId, timeout });
      ...
      const elapsed = Date.now() - startTime;
      log.info(`generateObject success`, { model: currentModelId, attempt, duration: elapsed });
      ...
      lastError = error as Error;
      log.warn(`generateObject attempt ${attempt} failed for model ${currentModelId}`, { 
        model: currentModelId, 
        error: (error as Error).message 
      });
