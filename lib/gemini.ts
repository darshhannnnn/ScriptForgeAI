...
export async function generateWithRetry(
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      const modelName = currentModel?.model || 'unknown';
      console.error(`Attempt ${i + 1} failed for model ${modelName}:`, error);
      ...
      // Detect transient error or model availability issues (503 / 500 / 429 / Service Unavailable / High demand / Resource exhausted / Not Found / 404)
      ...
      // Attempt fallback model if transient or model error occurs
      if (isFallbackTrigger && currentModel?.model) {
        ...
        if (nextModelName) {
          console.log(`[Gemini] Model ${currentModelName} failed. Falling back to ${nextModelName}...`);
          try {
            const client = getGeminiClient(currentModel.apiKey);
            currentModel = client.getGenerativeModel(
              {
                model: nextModelName,
            ...
            console.error(`[Gemini] Failed to instantiate fallback ${nextModelName}:`, fbErr);
          }
        }
      }
